...
.byte $F2                                  ; signature
.byte $56                                  ; signature
.byte NUM_8K_BLOCKS + 1                    ; length of program in consecutive 8K flash blocks. The +1 is the block for the loader
...
.text "allchars"                           ; Name of the program used for starting from onboard flash.
.byte $00                                  ; zero termination for "mless"
.byte $00                                  ; no parameter description, i.e. an empty string
.text "Print all characters. Type xgo after BASIC starts" ; Comment shown in lsf
...
; add more blocks if your BASIC program is longer
...
updateBlocks
    ...
    ldy #0
    lda 13
    ; skip the block in which the loader, i.e. this program, lives
    ina
_loop
    sta COPY_TAB, y
    iny
...
; pad the binary out to 8192 bytes
END_PROG
    .fill LOAD_ADDRESS + 8192 - END_PROG - 1
    .byte 0